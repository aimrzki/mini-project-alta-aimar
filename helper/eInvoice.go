...
func GetEmailBody(ticket model.Ticket, totalCost int, hotelName, hotelRoom string, kodeVoucher string, pointsEarned, usedPoints int) string {
	...
	emailBody += "</div>"
	emailBody += "<hr>"
	emailBody += "<div class='thank-you-message' style='background-color: #f5f5f5; padding: 20px; margin-top: 20px; border-radius: 5px; box-shadow: 0px 2px 4px rgba(0, 0, 0, 0.1);'>"
	emailBody += "<p style='font-size: 18px; margin: 10px 0;'>Terima kasih atas pembelian Anda!</p>"
	emailBody += "<p style='font-size: 18px; margin: 10px 0;'>Kami berharap Anda memiliki pengalaman yang menyenangkan di hotel kami.</p>"
	emailBody += "<p style='font-size: 18px; margin: 10px 0;'>Jangan Lupa untuk melakukan pembayaran di tempat, agar reward point dapat diterima</p>"
	emailBody += "</div>"
